...
XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=TWD&To=USD", headers=my_headers).content, "lxml")
...
for url in start_URLs:
	...
	links = ("https://www.mcdelivery.com.tw/tw/browse/menu.html" + a["href"] + "&locale=en" for a in (first_page.select("li.secondary-menu-item:not([class*='selected']) a[href]")))
	...
	for products in first_page.select("div.product-card"):
		...
		product["Price (TWD)"] = float((re.findall(r"[-+]?(?:\d*\.\d+|\d+)",products.select("span.starting-price")[0].text)[0]))
		product["Price (USD)"] = round((product["Price (TWD)"] * exchange_rate), 2)
