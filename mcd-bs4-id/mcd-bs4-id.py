...
try:
	...
	XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=IDR&To=USD",
	...
	print(
		f"1 IDR = {exchange_rate} USD (1 USD = {1/exchange_rate} IDR) on {local_datetime.strftime('%A, %-d %B %Y')}"
	)
	...
	for url in start_URLs:
	    ...
	    links = ("https://www.mcdelivery.co.id/id/browse/menu.html" + a["href"] + "&locale=en" for a in (
	    ...
	    for products in first_page.select("div.product-card"):
	        ...
	        product["Price (IDR)"] = float((re.findall(r"[-+]?(?:\d*\,\d+|\d+)",
	                                                   products.select("span.starting-price")[0].text)[0]).replace(",", ""))
	        product["Price (USD)"] = round(
	            (product["Price (IDR)"] * exchange_rate), 2)
