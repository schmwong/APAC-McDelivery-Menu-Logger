...
try:
    ...
    XE = BS(
        session.get(
            "https://www.xe.com/currencyconverter/convert/?Amount=1&From=VND&To=USD",
            headers=my_headers
        ).content,
        "lxml"
    )
    ...
    print(
        f"1 VND = {exchange_rate} USD (1 USD = {1 / exchange_rate} VND) on {local_datetime.strftime('%A, %-d %B %Y')}"
    )
    ...
    start_URLs = [
        "https://mcdelivery.vn/vn/browse/menu.html?daypartId=1&catId=1",  # Regular Menu
        "https://mcdelivery.vn/vn/browse/menu.html?daypartId=2&catId=1"  # Breakfast Menu
    ]
    ...
    # Function to scrape menu data from each page and populate the product_list
        ...
        for products in first_page.select("div.product-card"):
            ...
            product["Price (VND)"] = float(
                (
                    re.findall(
                        r"[-+]?(?:\d*,\d+|\d+)",
                        products.select("span.starting-price"
                                        )[0].text)[0]
                ).replace(",", "")
            )
            product["Price (USD)"] = round(
                (product["Price (VND)"] * exchange_rate), 2)
    ...
    for url in start_URLs:
        ...
        links = [
            f"https://mcdelivery.vn/vn/browse/menu.html{a['href']}" for a in (
