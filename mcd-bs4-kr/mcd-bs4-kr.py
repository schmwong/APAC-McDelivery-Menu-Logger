...
# Reflects local time
local_datetime = dt.datetime.now(pytz.timezone("Asia/Seoul"))
...
XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=KRW&To=USD", headers=my_headers).content, "lxml")
...
start_URLs = [     
    "https://www.mcdelivery.co.kr/kr/browse/menu.html?daypartId=1&catId=10&locale=en", # Regular Menu
		"https://www.mcdelivery.co.kr/kr/browse/menu.html?daypartId=2&catId=10&locale=en"  # Breakfast Menu
]
...
for url in start_URLs:
	...
	links = ("https://www.mcdelivery.co.kr/kr/browse/menu.html" + a["href"] + "&locale=en" for a in (first_page.select("li.secondary-menu-item:not([class*='selected']) a[href]")))
	...
	for products in first_page.select("div.product-card"):
		...
		product["Price (KRW)"] = float((re.findall(r"[-+]?(?:\d*\,\d+|\d+)",products.select("span.starting-price")[0].text)[0]).replace(",", ""))
		product["Price (USD)"] = round((product["Price (KRW)"] * exchange_rate), 2)
...
# Second Outer Loop iterates through list of generated URLs
for url in URL_list:
	next_page = BS(session.get(url, headers=my_headers).content,"lxml")

  # Second Inner loop iterates through elements on all other pages
...
output_file = str(timestamp + " mcd-bs4-kr.csv")
...
# Output filename format: "[YYYY-MM-DD hh:mm:ss] mcd-bs4-kr.csv"