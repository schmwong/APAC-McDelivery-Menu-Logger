...
try:
	...
	XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=KRW&To=USD",
	...
	print(
		f"1 KRW = {exchange_rate} USD (1 USD = {1/exchange_rate} KRW) on {local_datetime.strftime('%A, %-d %B %Y')}"
	)
	...
	start_URLs = [
	    ...
	    # Breakfast Menu
	    "https://www.mcdelivery.co.kr/kr/browse/menu.html?daypartId=2&catId=10&locale=en"
	]
	...
	for url in start_URLs:
	    ...
	    links = ("https://www.mcdelivery.co.kr/kr/browse/menu.html" + a["href"] + "&locale=en" for a in (
	    ...
	    for products in first_page.select("div.product-card"):
	        ...
	        product["Price (KRW)"] = float((re.findall(r"[-+]?(?:\d*\,\d+|\d+)",
	                                                   products.select("span.starting-price")[0].text)[0]).replace(",", ""))
	        product["Price (USD)"] = round(
	            (product["Price (KRW)"] * exchange_rate), 2)
