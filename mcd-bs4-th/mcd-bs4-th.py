...
try:
	...
	start_URLs = [
	    # All menus are on one page
	    "https://www.mcdonalds.co.th/mcDelivery/nonMember/category/promotion&lang=en"
	]
	...
	breakfast_page = BS(session.get(
	    "https://www.mcdonalds.co.th/mcDelivery/nonMember/category/breakfast", headers=my_headers).content, "lxml")
	...
	for url in start_URLs:
	    ...
	    links = (a["href"] for a in (first_page.select(
	        "ul.food-head-list > div.desktop > li:not([class*='selected']) a[href]")))
	    ...
	    for products in first_page.select("div.food-container > div.grid"):
	        # for selecting specific price tags in (CSS ":nth-child()" selector), and to limit the While Loop's iterations
	        i = 1
	        ...
	        for prices in products.select("div.selling-price"):
	            ...
	            while i <= len(products.select("div.selling-price")):
	                ...
	                if "Price" in products.select("div.price div.grid-label")[0].text:
	                    ...
	                    product["Menu Item"] = ((products.select("div.name")[0].text) + f" " + f"(" + (
	                        products.select(f".price > div:nth-child({i}) > div.grid-label")[0].text) + f")")
	                product["Price (THB)"] = round(float(re.findall(r"[-+]?(?:\d*\.\d+|\d+)", products.select(
	                    f".price > div:nth-child({i}) > div.selling-price")[0].text)[0]), 2)
	                product["Price (USD)"] = round(
	                    (product["Price (THB)"] * exchange_rate), 2)
	                product["Category"] = first_page.select(
	                    "div.food-container > div.section-head")[0].text.strip()
