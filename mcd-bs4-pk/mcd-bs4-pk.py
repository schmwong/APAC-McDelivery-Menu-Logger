...
try:
	...
	XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=PKR&To=USD",
	...
	for url in start_URLs:
	    ...
	    links = ("https://www.mcdelivery.com.pk/pk/browse/menu.html" + a["href"] for a in (
	    ...
	    for products in first_page.select("div.product-card"):
	        ...
	        product["Price (PKR)"] = float((re.findall(r"[-+]?(?:\d*\,\d+|\d+)",
	                                                   products.select("span.starting-price")[0].text)[0]).replace(",", ""))
	        product["Price (USD)"] = round(
	            (product["Price (PKR)"] * exchange_rate), 2)
