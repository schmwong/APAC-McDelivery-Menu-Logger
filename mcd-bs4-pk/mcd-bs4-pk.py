...
# Reflects local time (PKT)
local_datetime = dt.datetime.now(pytz.timezone("Asia/Karachi"))
...
XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=PKR&To=USD", headers=my_headers).content, "lxml")
...
start_URLs = [     
    "https://www.mcdelivery.com.pk/pk/browse/menu.html?daypartId=1&catId=12", # Regular Menu
		"https://www.mcdelivery.com.pk/pk/browse/menu.html?daypartId=6&catId=21", # Midnight Menu
		"https://www.mcdelivery.com.pk/pk/browse/menu.html?daypartId=2&catId=12"  # Breakfast Menu
]
...
for url in start_URLs:
	...
	links = ("https://www.mcdelivery.com.pk/pk/browse/menu.html" + a["href"] for a in (first_page.select("li.secondary-menu-item:not([class*='selected']) a[href]")))
	...
	for products in first_page.select("div.product-card"):
		...
		product["Price (PKR)"] = float((re.findall(r"[-+]?(?:\d*\,\d+|\d+)",products.select("span.starting-price")[0].text)[0]).replace(",", ""))
		product["Price (USD)"] = round((product["Price (PKR)"] * exchange_rate), 2)
...
output_file = str(timestamp + " mcd-bs4-pk.csv")
...
# Output filename format: "[YYYY-MM-DD hh:mm:ss] mcd-bs4-pk.csv"