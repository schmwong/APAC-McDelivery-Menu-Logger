...
XE = BS(session.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=JPY&To=USD", headers=my_headers).content, "lxml")
...
start_URLs = [     
    "https://mcdelivery.mcdonalds.com/jp/browse/menu.html?daypartId=1&catId=1&locale=en", # Regular Menu
		"https://mcdelivery.mcdonalds.com/jp/browse/menu.html?daypartId=2&catId=1&locale=en"  # Breakfast Menu
]
...
for url in start_URLs:
	...
	links = ("https://mcdelivery.mcdonalds.com/jp/browse/menu.html" + a["href"] + "&locale=en" for a in (first_page.select("li.secondary-menu-item:not([class*='selected']) a[href]")))
	...
	for products in first_page.select("div.product-card"):
		...
		# Returns list: ['onProductClick({ \'name\':"Spicy Double Thick Beef"', "'id':'1535'", "'price':'560.0'", "'brand':'McDonalds'", '\'cat\':"Recommendations"', "'variant':''", "'url':'#signin'})"]
		details = (products.select("div.product-controls > a[onclick]")[0].attrs["onclick"]).split(",")
		# Price is the third element (index [2])
		# Extract only the digits and decimal point, returns a string
