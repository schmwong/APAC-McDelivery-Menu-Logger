...
try:
	...
	for url in start_URLs:
	    ...
	    links = ("https://mcdelivery.mcdonalds.com/jp/browse/menu.html" + a["href"] + "&locale=en" for a in (
	    ...
	    for products in first_page.select("div.product-card"):
	        ...
	        details = (products.select(
	            "div.product-controls > a[onclick]")[0].attrs["onclick"]).split(",")
	        # Price is the third element (index [2])
	        # Extract only the digits and decimal point, returns a string
	        # Convert string to decimal number
	        product["Price (JPY)"] = float(re.findall(
	            r"[-+]?(?:\d*\.\d+|\d+)", details[2])[0])
	
	        product["Price (USD)"] = round(
	            (product["Price (JPY)"] * exchange_rate), 2)
